...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`services_technical_specs\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`label\` text NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`services_technical_specs_order_idx\` ON \`services_technical_specs\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`services_technical_specs_parent_id_idx\` ON \`services_technical_specs\` (\`_parent_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`services_sidebar_inclusions\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`services_sidebar_inclusions_order_idx\` ON \`services_sidebar_inclusions\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`services_sidebar_inclusions_parent_id_idx\` ON \`services_sidebar_inclusions\` (\`_parent_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`services\` (
  	...
  	\`tagline\` text,
  	\`marketing\` text,
  	\`editorial_quote_quote\` text NOT NULL,
  	\`editorial_quote_citation\` text,
  	\`meta_title\` text,
  ...
  await db.run(sql`CREATE TABLE \`leads\` (
  	...
  	\`source\` text DEFAULT 'service-detail',
  	\`updated_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  	\`created_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  	FOREIGN KEY (\`service_requested_id\`) REFERENCES \`services\`(\`id\`) ON UPDATE no action ON DELETE set null
  );
  `)
  await db.run(sql`CREATE INDEX \`leads_service_requested_idx\` ON \`leads\` (\`service_requested_id\`);`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`services_technical_specs\`;`)
  await db.run(sql`DROP TABLE \`services_inclusions\`;`)
  await db.run(sql`DROP TABLE \`services_sidebar_inclusions\`;`)
